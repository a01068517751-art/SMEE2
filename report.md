...
사용자 `u`와 anchor `i`에 대해 실제 거리는 다음과 같다.

| Symbol | Definition |
|---|---|
| `d_true,i(u)` | `sqrt((x_u - x_i)^2 + (y_u - y_i)^2)` |
| `e_i(u)` | `d_hat_i(u) - d_true,i(u)` |

Anchor `i`의 bias는 평균 대신 median으로 계산하였다.
...
| Quantity | Definition |
|---|---|
| `sigma_i` | `std((d_hat_i - d_true,i) - b_i)` |
...
$$
p_k=\arg\min_p \sum_{i\in S_k} w_i\left(\|p-a_i\|-d_{\mathrm{cal},i}\right)^2
$$
...
| Quantity | Definition |
|---|---|
| `r_i(p_k)` | residual for candidate position `p_k` |
| `J(p_k)` | robust weighted cost |

$$
r_i(p_k)=\lVert p_k-a_i\rVert-d_{\mathrm{cal},i}
$$

$$
J(p_k)=\sum_i w_i\rho\!\left(r_i(p_k)\right)
$$
...
|---|---|
| `tau_i` | gating threshold |
| `GATE_SCALE` | `3.10` |
| Gate condition | `abs(r_i) <= tau_i` |

$$
\tau_i = GATE\_SCALE\cdot\sigma_i
$$
...
$$
p_{\mathrm{base}}=\arg\min_p \sum_i w_{\mathrm{final},i}\left(\|p-a_i\|-d_{\mathrm{cal},i}\right)^2
$$

반복은 위치 변화량이 충분히 작아지거나 최대 반복 횟수에 도달하면 종료하였다. 이 결과 `p_base`는 물리 기반 robust localization pipeline이 산출한 기본 위치이다.
...
| Objective | Meaning |
|---|---|
| `min_W norm(Y - XW)^2 + alpha * norm(W)^2` | coefficient가 과도하게 커지는 것을 억제하면서 residual correction 학습 |
...
| Metric | Definition |
|---|---|
| RMSE | `sqrt((1/N) * sum_i norm(p_i - p_hat_i)^2)` |
| MAE | `(1/N) * sum_i norm(p_i - p_hat_i)` |
...
| Bounded Ridge compensation | `p_base`의 residual pattern으로 `dx`, `dy` 제한 보정 | robust WLS 이후 남는 systematic error 감소 |
...
| Lambda mixture 적용 | `p_base`를 완전히 대체하지 않고 제한적으로 보정 |
| `p_base` residual feature 사용 | direct position learning보다 물리 기반 구조 유지 |
| `model_ridge.npz` 저장 | 학습된 상수와 결과 재현성 확보 |
| `main.py`에서 추가 학습 없음 | hidden test에서 label을 사용하지 않고 추론만 수행 |
